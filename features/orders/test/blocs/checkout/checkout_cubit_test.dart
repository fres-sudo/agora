...
void main() {
  ...
  group('CheckoutCubit', () {
    ...
    test(
      'cash: canConfirm only when tendered covers total; change computed',
      () {
    ...
    blocTest<CheckoutCubit, CheckoutState>(
      ...
      setUp: () {
        ...
        when(
          productsRepository.getProductById(1),
        ).thenAnswer((_) async => Result.ok(product(id: 1, trackStock: false)));
      },
      act: (cubit) {
        cubit.start(buildCart());
        cubit.setTendered(1000);
    ...
    group('receipt printing (P2-3)', () {
      ...
      test(
        'success state carries a rendered receipt for the preview',
        () async {
          stubSuccessfulSale();
      ...
      test('printReceipt sends bytes and marks status printed', () async {
        ...
        expect(
          (cubit.state as CheckoutSuccess).printStatus,
          PrintStatus.printed,
        );
        await cubit.close();
      });
      ...
      test(
        'printReceipt marks status failed when the printer errors',
        () async {
          stubSuccessfulSale();
